...
func TestPgConstraintMessage(t *testing.T) {
	tests := []struct {
		...
		{"known unique constraint, currency price observation", &pgconn.PgError{Code: "23505", ConstraintName: "currency_prices_unique_observation"}, "a rate for this currency pair at this exact instant already exists"},
		{"known fkey constraint, currency price base", &pgconn.PgError{Code: "23503", ConstraintName: "currency_prices_base_currency_id_fkey"}, "referenced currency does not exist"},
		{"known fkey constraint, currency price quote", &pgconn.PgError{Code: "23503", ConstraintName: "currency_prices_quote_currency_id_fkey"}, "referenced currency does not exist"},
