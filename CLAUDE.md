...
**Accounts** support a self-referencing `parent_id` for a chart-of-accounts hierarchy; deletes are `ON DELETE RESTRICT` (children/entries block deleting a parent/account). An account's `balance` is never stored — `AccountStore.List`/`Get` compute it on every read via a `LEFT JOIN` against `transaction_entries` (see `accountWithBalanceQuery`), and it's always just that account's own entries, with no roll-up from children. `AccountStore.Ledger` (behind `GET /accounts/{id}/transactions`) similarly computes a running balance per account via a SQL window function (`SUM(...) OVER (ORDER BY timestamp, id)`), rather than the TUI or any Go code summing it. The TUI's accounts list renders the hierarchy as an indented tree (`orderAccountsAsTree` in `tui/internal/ui/accounts.go`): each account immediately followed by its children, siblings at every level ordered by ID; pressing `Enter` on a row drills into that account's ledger view.
